...
void remplir(int modifier, int supprimer,sclient *client){ /*Fonction faite par Rémi JARA*/
    ...
    if (modifier == 0 && supprimer == 0){
        ...
        while(fgets(ligne,TAILLE_MAX+1,fichier)){/* on récupère chaque ligne du fichier dans la variable ligne*/
            ...
            client[i].deleted = strdup("ok"); /*remplissage de l'id du client et de son état de suppression*/
            client[i].id = i+1;
            i++;
        }
...
void ajout(){ /*Fonction faite par Rémi JARA*/
    ...
    tableau[i].adresse = strdup(adresse);/*on remplit à la dernière place du tableau chaque attribut du client avec ceux scanés*/
...
int est_champ(char * choix_filtre){/*fonction qui renvoit 1 si la valeur passé en argument est un champ valide, 0 sinon*/
...
int est_zero_ou_un(char *choix){/*fonction qui renvoit 1 si la valeur passé en argument est un 0 ou un 1, renvoit 0 sinon*/
